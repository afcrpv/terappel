class Search < ActiveRecord::Base
  ...
  def search_params
    ...
    result['produits'] = Produit.find(produit_ids).map(&:name).to_sentence(two_words_connector: ' ou ', words_connector: ' ou ', last_word_connector: ' ou ') if produit_ids
    result['dcis'] = Dci.find(dci_ids).map(&:libelle).to_sentence(two_words_connector: ' ou ', words_connector: ' ou ', last_word_connector: ' ou ') if dci_ids
    ...
    result = result.delete_if { |k, _v| k == 'local' }
    result = result.delete_if { |k, _v| k =~ /id$/ }
    result = result.delete_if { |k, _v| %w(max_date_appel min_date_appel).include?(k) }
    result = result.delete_if { |k, _v| %w(created_at updated_at).include?(k) }
    result = result.delete_if { |_k, v| !v.present? }
    result
  end
end
